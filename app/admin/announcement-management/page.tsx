...
export default function AnnouncementManagementPage() {
  ...
  const fetchAnnouncements = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const result = await response.json()
        console.error('獲取活動更新列表失敗:', result.error || '未知錯誤')
      }
    } catch (error) {
  ...
  const handleBatchDelete = async () => {
    ...
    if (!confirm(`確定要刪除 ${selectedItems.length} 個活動更新嗎？`)) {
      return
    }

    try {
      const deletePromises = selectedItems.map(id => apiDelete(`/api/announcements/${id}`))
  ...
  // 顯示資料（根據搜尋關鍵字篩選）
  const displayData = announcements.filter((announcement) => {
    ...
    const announcementId = announcement.announcementId.toLowerCase()
    const content = (announcement.content || '').toLowerCase()
    const type = (announcement.type || '').toLowerCase()
    
    return (
      title.includes(keyword) ||
      announcementId.includes(keyword) ||
          ...
          <table className="w-full min-w-[1000px] divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200 whitespace-nowrap w-[300px]">
                  內容
                </th>
              ...
              {loading && !dataLoaded ? (
                ...
                displayData.map((item, index) => (
                  <tr key={item.id} className={`hover:bg-gray-100 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                    ...
                    <td className="px-6 py-4 text-center border-r border-gray-200 text-gray-900">
