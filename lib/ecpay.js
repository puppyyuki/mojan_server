...
function createEcpayPaymentData(
  ...
  // Flutter Web 應用 URL（用戶返回的頁面）
  // 優先使用環境變數，否則使用預設的 Flutter 應用 URL
  const flutterAppUrl = process.env.FLUTTER_APP_URL || process.env.FRONTEND_URL || 'https://mojan-app.onrender.com';
  ...
  console.log('   flutterAppUrl:', flutterAppUrl);
  ...
  const basePaymentData = {
    ...
    // 返回 Flutter Web 應用
    // 使用 HashRouter (#/shop) 以避免伺服器路由問題
    // 對於 Web：使用 #/shop（HashRouter，不依賴伺服器配置）
    // 對於 iOS/Android：WebView 會自動處理，不會觸發外部瀏覽器
    ClientBackURL: `${flutterAppUrl}#/shop`,
    OrderResultURL: `${flutterAppUrl}#/shop`,
...
function verifyCheckMacValue(data) {
  try {
    // 複製資料以避免修改原始物件
    const dataCopy = { ...data };
    const receivedCheckMacValue = dataCopy.CheckMacValue;
    delete dataCopy.CheckMacValue; // 移除 CheckMacValue 再計算
    
    const calculatedCheckMacValue = generateCheckMacValue(dataCopy);
    
    // 調試輸出（僅在驗證失敗時）
    if (receivedCheckMacValue !== calculatedCheckMacValue) {
      console.log('🔍 CheckMacValue 驗證調試:');
      console.log('   收到的值:', receivedCheckMacValue);
      console.log('   計算的值:', calculatedCheckMacValue);
      ...
      console.log('   用於計算的參數:', JSON.stringify(filteredParams, null, 2));
      ...
      const paramString = sortedKeys
        ...
        .join('&');
      const rawString = `HashKey=${ecpayConfig.HashKey}&${paramString}&HashIV=${ecpayConfig.HashIV}`;
      console.log('   參數字串:', paramString);
      console.log('   原始字串:', rawString);
      console.log('   HashKey:', ecpayConfig.HashKey);
      console.log('   HashIV:', ecpayConfig.HashIV);
    }
