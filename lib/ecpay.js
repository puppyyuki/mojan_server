...
function createEcpayPaymentData(
  ...
  const basePaymentData = {
    ...
    // 返回 Flutter Web 應用（Flutter Web 使用標準路由）
...
function createEcpayPaymentForm(
  ...
  // 建立 HTML 表單 - 站內付2.0格式
  // 添加完整的 HTML 結構和 viewport meta tag 以支援手機顯示
  let formHtml = `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
      ...
      <form id="ecpay-form" method="post" action="${getEcpayPaymentUrl()}" style="display:none;">
  `;
